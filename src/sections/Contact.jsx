...
export default function ContactForm() {
  ...
  const handleSubmit = async (e) => {
    ...
    const response = await fetch("https://api.web3forms.com/submit", {
      ...
      body: JSON.stringify({
        access_key: "780b6ad8-8666-4be5-9798-03b9e39f5cd8", // 🔑 Get from web3forms.com
        ...formData
      })
    });
  ...
  return (
    <section id="contact" className=" bg-gray-900 text-white py-20 px-6 md:px-20">
      <h2 className=" text-4xl font-bold text-center mb-12 text-[#ff004f]">Contact Me</h2>
      <form onSubmit={handleSubmit} className="space-y-4">
        <input
          ...
          required
          className="w-full px-4 py-3 bg-gray-700 text-white rounded-md border border-gray-600 focus:outline-none focus:ring-2 focus:ring-[#ff004f] transition"
        />
        <input
          type="email"
          name="email"
        ...
        <button
          type="submit"
          className="w-full bg-[#ff004f] hover:bg-pink-700 transition-colors duration-300 text-white font-semibold py-3 rounded-md shadow-md"
        >
          Send Message
        </button>
      </form>
